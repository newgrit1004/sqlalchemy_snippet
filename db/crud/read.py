...
class SingleTableSQLInterface:
    def __init__(self, columns:List[str], SessionLocal:sessionmaker, orm_class:Generic[T]):
    ...
    def check_order_by_condition(self, **kwargs)->Union[Callable, None]:
        ...
        if kwargs.get('order_by_condition') is None:
            ...
            return getattr(getattr(self.orm_cls, kwargs.get('order_by_condition').get('column', 'acq_time')),
    ...
    def check_between_condition(self, **kwargs):
        ...
        if kwargs.get('between_condition') is None:
            return []
        return [getattr(self.orm_cls, kwargs.get('between_condition').get('column')) > kwargs.get('between_condition').get('from'), getattr(self.orm_cls, kwargs.get('between_condition').get('column')) < kwargs.get('between_condition').get('end')]

    def load_query_result(self, **kwargs)->List[Dict[str, Any]]:
        with self.local_session() as session:
            query_results = (
                session.query(*[getattr(self.orm_cls, column) for column in self.columns])
